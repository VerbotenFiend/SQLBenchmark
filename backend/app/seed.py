...
def wait_for_db(retries=60, delay=2.0):
    ...
    sys.exit(1)
#controllo se in film c'è qualcosa, in caso è gia popolato
def db_has_data():
    conn = get_connection()
    try:
        cur = conn.cursor()
...
#apre tsv, check header (tenta la conversione della col 3,4 in numeri), crea una line csv e chiama add
def seed_from_tsv(tsv_path: str):
    ...
    with open(tsv_path, "r", encoding="utf-8") as f:
        ...
        for idx, row in enumerate(rows_iter, start=1):
            ...
            # se il tuo TSV ha più/meno colonne, adatta qui
            if len(row) < 5:
                errors += 1
                print(f"[riga {idx}] colonne insufficienti: {row}", file=sys.stderr)
                continue
            # normalizza a 7 campi riempiendo p1/p2 vuoti se mancano
            row = (row + ["", ""])[:7]
            titolo, regista, eta, anno, genere, p1, p2 = [c.strip() for c in row]

            # costruisci la data_line CSV attesa da add_line (7 campi separati da virgola)
